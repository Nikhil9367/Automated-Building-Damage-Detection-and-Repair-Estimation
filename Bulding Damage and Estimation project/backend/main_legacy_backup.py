...
def detect_damage_with_gemini(image_path):
    ...
    try:
        ...
        prompt = f"""
        You are a deterministic, 99%-accuracy structural damage analysis system.
        ...
        1. IMAGE FINGERPRINT LOCK (MANDATORY)
        ...
        4. FIXED COST ENGINE & GUIDELINES (India 2024-2025 Rates)
...
def enrich_report_data(basic_result):
    ...
    try:
        ...
        if "crack" in damage_type:
            ...
            det_action = "Conduct on-site inspection."
        ...
        enriched["detailed_assessment"] = {
            ...
            "location": "Vertical Wall Element (Est.)",
            "description": det_desc,
            "risk_level": "High" if severity > 0.7 else "Medium",
        ...
        # Materials
        mats = []
        # materials_db is likely a dict with "materials" key based on inspection
        real_materials = materials_db.get("materials", []) if isinstance(materials_db, dict) else materials_db
        
        # Find a material from DB that matches logic or just pick first few relevant
        # Filter DB for "Repair" or "Epoxy" or "Chemical"
        candidate_mats = []
        for m in real_materials:
            item_name = m.get("item", "").lower()
            # Loose matching
            if "repair" in item_name or "chemical" in item_name or "epoxy" in item_name or "grout" in item_name or "mortar" in item_name:
                candidate_mats.append(m)
                
        if not candidate_mats: candidate_mats = real_materials[:2]
        
        total_mat_cost = 0
        for m in candidate_mats[:2]: # Take top 2
            qty = int(5 * multiplier)
            # Use price_with_gst or base_price. Key is 'item', 'price_with_gst'
            rate = m.get("price_with_gst", m.get("base_price", 0))
            cost = int(qty * rate)
            total_mat_cost += cost
            
            # Extract unit from item name if possible "Name (Unit)"
            name = m.get("item", "Unknown Material")
            unit = "Unit"
            if "(" in name and ")" in name:
                try:
                    unit = name.split("(", 1)[1].rsplit(")", 1)[0]
                except:
                    pass
        ...
        # Equipment
        eqs = []
        # Refined Logic based on Damage Type
        keyword_filters = []
        if "crack" in damage_type.lower():
            keyword_filters = ["safety"]
            # Only add scaffolding if severity is high (implying large/high crack)
            if severity > 0.6:
                keyword_filters.append("scaffold")
        elif "spall" in damage_type.lower() or "structural" in damage_type.lower():
            keyword_filters = ["safety", "drill", "cutter", "scaffold"]
        else:
            keyword_filters = ["safety"]
        ...
        for e in equipment_db:
            e_name = e["name"].lower()
            if any(k in e_name for k in keyword_filters):
                candidate_eq.append(e)
        ...
        enriched["post_repair_life_expectancy"] = {
             ...
             "life_extension_years": "+5 to 10 Years",
             "warranty_period": "1 Year on workmanship",
             "maintenance_recommendation": "Annual inspection recommended.",
             "improvement_summary": "Structural integrity restored and further propagation arrested."
        }
        
        enriched["final_recommendations"] = {
            "structural_risk": "Reduced from Moderate to Low after repair.",
            "repair_urgency": " Recommended within 1 month.",
            "future_prevention_steps": "maintain drainage and seal surface cracks periodically."
        }
...
def generate_damage_pdf(inspection_record, out_path):
    ...
    data = [
        ...
        ["Confidence", str(res.get('confidence', 'N/A'))],
        ["Bounding Box", str(res.get('bbox', 'N/A'))],
...
def generate_remediation_pdf(inspection_record, out_path):
    ...
    tech_data = [
        ...
        [p("Confidence"), p(res.get('confidence', '0.99 (AI Assessment)'))], 
        [p("Bounding Box"), p(res.get('bbox', [0,0,0,0]))],
    ]
